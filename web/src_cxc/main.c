...
int generate(FILE *f, const char *filename, long length, char *ctemp,
    ...
    if (props_start != NULL) {
        ...
        if (props_end != NULL) {
            ...
            strncat(full_props_block, props_start, props_end - props_start);

            char *line = strtok(full_props_block, "\n");
            while (line != NULL) {
                if (strstr(line, "#include") != NULL || strstr(line, "typedef") != NULL) {
                    strcat(h_prepend, line);
                    strcat(h_prepend, "\n");
                } else {
                    strcat(props_members, line);
                    strcat(props_members, "\n");
                }
                line = strtok(NULL, "\n");
            }
        }
